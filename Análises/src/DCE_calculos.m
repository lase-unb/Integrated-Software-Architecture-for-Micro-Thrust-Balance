...
DE = 0.001; % Distância longitudinal entre os eletrodos (gap de separação)
...
d_testes = [0.010 : 0.005 : 0.030, 0.0318];
...
for i = 1:length(d_testes)
    ...
    plot(U, FE, 'LineWidth', 1.5, 'DisplayName', sprintf('D = %.1f mm', d * 1000));
end

%% 5. MARCAÇÃO DOS LIMITES DE PROJETO
% Desenha linhas horizontais vermelhas tracejadas para mostrar a janela de operação
yline(FE_min, '--r', 'F_{min} (20 \muN)', 'LineWidth', 1.5, 'LabelHorizontalAlignment', 'left', 'HandleVisibility', 'off');
yline(FE_max, '--r', 'F_{max} (20 mN)', 'LineWidth', 1.5, 'LabelHorizontalAlignment', 'left', 'HandleVisibility', 'off');
...
title('Força vs Tensão para Diferentes Diâmetros e DE = 1 mm', 'FontSize', 14);
