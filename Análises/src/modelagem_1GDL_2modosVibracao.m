...
% 1. Parâmetros físicos do pêndulo (Dados de Entrada Atualizados)
g = 9.784;       % Gravidade local. Coordenadas: (-15.999557, -48.046830).
m_p = 0.75935;   % [kg] (massa do propulsor + base)
L_p = 0.368;     % [m] (distância do propulsor até o pivô)
m_c = 0;   
L_c = 0;
L_s = 0.368;
zeta = 0.00029;
...
K_eq_theta = 2347.14787e-6 / (L_s^2);
...
amp_pre = 0;        % Amplitude prévia de 0.2 micrometros

% 4. Configurações de Força e Tempo
F0 = 8.25259e-6;    % Força de microN
t_pulse = 10e-6;    % Duração de 10 microsegundos

t_evento = 20;      % Instante em que a perturbação ocorre
t_total = 300;      % Tempo total de simulação estendido
...
x_init_evento = [pos_pre(end); vel_pre(end)];
...
x_init_livre = x_init_evento + [1e-6; 0]; 
...
[~, x2_post] = ode45(@(t,x) A*x + B*F0, t_post, x_init_evento);
...
[~, x3_a] = ode45(@(t,x) A*x + B*F0, [t_evento, t_evento + t_pulse], x_init_evento);
...
grid on;
ylabel({'Deslocamento', '(\mu m)'});
title('2. Força Contínua (Degrau de 8.25259 \mu N aplicado em t=20s)');
xlim([0 t_total]);
...
xlabel('Tempo (s)');
ylabel({'Deslocamento', '(\mu m)'});
title('3. Pulso Único (8.25259 \mu N por 10 \mu s em t=20s)');
xlim([0 t_total]);
